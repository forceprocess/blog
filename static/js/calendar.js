(function (factory) {
    ...
    Calendar.prototype.init = function(){
        ...
        var calendar = getElement(document, '#calendar');
        if(!calendar) return false;

        calendar.appendChild(oCalenWrap);
        oCalenWrap.appendChild(oCalen);
        $this.toggle = attr(calendar, 'toggle') == 'true'?true:false;
     ...
     * @return {[type]}        [description]
     */
    Calendar.prototype.createCalenWeekList = function(data, setTitle){
        var oList = document.createElement('div'),
            created = 0,
            $this = this;
        ...
        data.m = data.m || 0;
        data.w = data.w || 0;
        var tDay = formatDate(new Date());          //今天
        
        var now = new Date(),
            nowYear = now.getFullYear(),            //当前年
            nowMonth = now.getMonth()+data.m,      //当前月
            nowDay = now.getDate()+data.w*7,       //当前日
            nowDayOfWeek = now.getDay();            //当前星期
        for (var i = 0; i < 7; i++) {
            var weekDate = new Date(nowYear, nowMonth, nowDay - nowDayOfWeek+i);
            var n = weekDate.getDate(),
                oSpan = create('span'),
                oNum = create('a', {
                    "data-calen" : formatDate(weekDate),
                    "href" : 'javascript:;'
                    }, n);

            switch(weekDate.getMonth() == nowMonth && created % 7 ){
                case 0: case 1: oNum.className = 'weekend'; break;
            }

            if(weekDate.getMonth() < nowMonth && weekDate.getMonth() < nowMonth ){
                toolClass(oNum, 'prev-m prev-to-month pasted');
            }

            if(weekDate.getMonth() > nowMonth && weekDate.getMonth() > nowMonth ){
                toolClass(oNum, 'next-m next-to-month');
            }

            if(formatDate(weekDate) == tDay){
                toolClass(oNum, 'today');
            }
     ...
     * @return {[type]}      [description]
     */
    Calendar.prototype.createCalendarList = function(cb){
        var $this = this;
        oCalen.appendChild(calendarList);

        var start = Number(attr(calendar, 'start')) || 2010,
            end = Number(attr(calendar, 'end')) || 2025;

        past = !(attr(calendar, 'past') == null);
        $this.hours = !(attr(calendar, 'hours') == null);
        $this.hoursPast = !(attr(calendar, 'hours-past') == null);

        $this.shield = getDate(attr(calendar, 'shield') || '');
        $this.startDate = getDate(attr(calendar, 'start-date') || '');
        ...
        if(!$this.startDate instanceof Array || !$this.startDate){
            ...
            if($this.toggle){
                calendarWeeklist = create('div', {"class" : 'calendar-weeklist close'}),    // 日历切换显示隐藏
                oCalen.appendChild(calendarWeeklist);
                oCalen.appendChild(calendarList);
                wNow = 0;
                $this.startWeekJSON.prev = { w : wNow - 1, m : mNow };
                $this.startWeekJSON.now = { w : wNow, m : mNow };
                $this.startWeekJSON.next = { w : wNow + 1, m : mNow };
            }else{
                $this.startWeekJSON = {};
            }
        ...
        // 创建日历对象列表
        $this.appendList($this.startJSON, $this.startWeekJSON, function(){
            $this.addEvent();
        });
        cb && cb();
    }  
     ...
     * @return {[type]}      [description]''
     */
    Calendar.prototype.appendList = function(data, data2, cb){
        data = data || {};
        ...
        if(data2){
           data2.prev = data2.prev || {m : mNow - 1, w : wNow};
           data2.now = data2.now || {m : mNow, w : wNow};
           data2.next = data2.next || {m : mNow + 1, w : wNow};

           calendarWeeklist.innerHTML = '';

           calendarWeeklist.appendChild(this.createCalenWeekList(data2.prev));
           calendarWeeklist.appendChild(this.createCalenWeekList(data2.now, true));
           calendarWeeklist.appendChild(this.createCalenWeekList(data2.next)); 
        }
    ...
    Calendar.prototype.addEvent = function(){
        ...
        for(var i = 0 ; i < aCalenSet.length ; i++){
            aCalenSet[i].onclick = function(){
	            ...
	            var dateArr = this.getAttribute('data-calen').split('/');
	            if(dateArr[1]==0){
                ...
                if(toolClass(this, 'prev-to-month', 'has')){
	                ...
	                setTimeout(function () {
		                ...
		                document.querySelector('#calendar').querySelector('.calendar-list').classList.remove('active');
		                var dom = document.querySelector('#calendar').querySelector('.calendar-list').getElementsByTagName('div')[1].getElementsByTagName('a');
		                for(var i=0; i<dom.length; i++){
			                var elem = dom[i];
			                if(elem.getAttribute('data-calen')==dateArr){
                    ...
                    var date = attr(this, 'data-calen'), today = this.innerHTML;
                        date = format(date, (attr(calendar, 'format') || false));

                    if($this.hours){
                        $this.createTime(calendar, date, today, past);
                    }
                    else {
                        if(calendar && typeof calendar.value == 'undefined'){
                            // calendar.innerHTML = date;
                            calendar.setAttribute('chooseDate',date);
                            window.xtag&&window.xtag.trigger&&window.xtag.trigger(calendar,'xchange',date);
                            ...
                            var type = typeof calendar.value;
                            if(type === 'string' || type === 'number'){
                                if(calendar.oldValue != date){
     ...
     * @param  {[type]} dir [上个月/周还是下个月/周]
     */
    Calendar.prototype.transitions = function(obj, dir,type){
        debugger;
        var $this = this;
        var json = {},json2 = {};
        if(!type){
           ...
           json2 = $this.startWeekJSON;
       }

       setTimeout(function(){
           end(json,json2);
       }, 500)
       function end(json,json2){
           $this.appendList(json, json2,function(){
